...
type CalculatorServiceClient interface {
	...
	MaxSoFar(ctx context.Context, opts ...grpc.CallOption) (CalculatorService_MaxSoFarClient, error)
}
...
func (c *calculatorServiceClient) MaxSoFar(ctx context.Context, opts ...grpc.CallOption) (CalculatorService_MaxSoFarClient, error) {
	stream, err := c.cc.NewStream(ctx, &CalculatorService_ServiceDesc.Streams[2], "/calculator.CalculatorService/MaxSoFar", opts...)
	if err != nil {
		return nil, err
	}
	x := &calculatorServiceMaxSoFarClient{stream}
	return x, nil
}

type CalculatorService_MaxSoFarClient interface {
	Send(*MaxRequest) error
	Recv() (*MaxResponse, error)
	grpc.ClientStream
}

type calculatorServiceMaxSoFarClient struct {
	grpc.ClientStream
}

func (x *calculatorServiceMaxSoFarClient) Send(m *MaxRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *calculatorServiceMaxSoFarClient) Recv() (*MaxResponse, error) {
	m := new(MaxResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type CalculatorServiceServer interface {
	...
	MaxSoFar(CalculatorService_MaxSoFarServer) error
...
func (UnimplementedCalculatorServiceServer) MaxSoFar(CalculatorService_MaxSoFarServer) error {
	return status.Errorf(codes.Unimplemented, "method MaxSoFar not implemented")
}
...
func _CalculatorService_MaxSoFar_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(CalculatorServiceServer).MaxSoFar(&calculatorServiceMaxSoFarServer{stream})
}

type CalculatorService_MaxSoFarServer interface {
	Send(*MaxResponse) error
	Recv() (*MaxRequest, error)
	grpc.ServerStream
}

type calculatorServiceMaxSoFarServer struct {
	grpc.ServerStream
}

func (x *calculatorServiceMaxSoFarServer) Send(m *MaxResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *calculatorServiceMaxSoFarServer) Recv() (*MaxRequest, error) {
	m := new(MaxRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var CalculatorService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "MaxSoFar",
			Handler:       _CalculatorService_MaxSoFar_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
